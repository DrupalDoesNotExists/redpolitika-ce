...
> Техническая спецификация CE-ядра redpolitika. Описывает архитектуру, доменную модель, протоколы, плагин-систему и дизайн-решения.

---

## 1. Назначение и концепция

**redpolitika** — open-core сервис проверки и правки текста по гибким правилам редакционной политики (редполитики) в YAML. Концептуально — «Главред, но для любой редполитики»: правила описываются в YAML, а не зашиты в код.
...
- **`exclude`** — сахар: child-совпадения минус список слов
...
```text
<!-- rp:disable rule-id --> ... <!-- rp:enable -->
строка <!-- rp:disable-line rule-id -->
```

Без rule-id подавляются все правила. `rp:disable-line` действует на всю строку. Комментарии исключаются из скоринга.
...
- Клиентские правила — detect использует только `regex`/`wordlist`/`contains`/`eq`/`prefix`/`suffix` (выполняются на фронтенде мгновенно)
- Серверные правила — любая «тяжёлая» стадия (LLM/NER/POS/плагин/expr с серверной функцией)
- Endpoint `/api/client-rules` отдаёт только клиентские
- `POST /api/analyze?full=true` форсирует полный анализ на сервере
...
`?full=true` — включить client-side правила.
...
|-----|---------|----------|
| `check` / `analyze` | `{ text, textHash, full }` | Запрос анализа текста (debounce 500ms) |
