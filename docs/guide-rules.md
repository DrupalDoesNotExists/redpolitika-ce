...
Правила — YAML-файлы на диске. Движок Редполитики загружает, валидирует и компилирует их при старте. Некорректные правила блокируют запуск.
...
Ссылается на другое правило — его дерево детекции подставляется вместо `ref`.
...
Полезно, когда несколько правил используют один и тот же набор совпадений: можно вынести общую детекцию в отдельное правило и переиспользовать через `ref`.
...
- Целевое правило должно существовать и иметь дерево детекции (нельзя сослаться на правило, использующее только серверные методы)
- Циклические ссылки (`A ссылается на B, B ссылается на A`) запрещены — загрузка упадёт с ошибкой
...
Возвращает совпадение с любым входным текстом.

```yaml
detect:
  # дерево сработает на любой входной текст
  any: {}
```
...
Движок Редполитики загружает правила из директорий (ничего не встроено в образ). Три слоя (deep-merge по `id`):
